...
% re-written by hannah
% Poisson distribution
\item \emph{Car dealership.} A car dealership keeps used cars that
...
month of June according to a Poisson distribution with a mean of four
hailstorms. After a hailstorm, the number of dents in a car follows a Poisson distribution with a mean of five dents.
\begin{enumerate}
	\item Determine the probability that there will be exactly five
	hailstorms at the dealership in June. \label{ex:storms}
	\item After a hailstorm, determine the probability that a car will have two or more dents. \label{ex:cars}
\end{enumerate}
...
\begin{solution}
	...
	follows a Poisson distribution with a mean of four
	hailstorms. Therefore, $X \sim \text{Poisson}(\lambda = 4)$.  The
	probability that there are exactly five hailstorms is
	\[
	P(X=5) = \frac{e^{-\lambda}\lambda^5}{5!} = \frac{e^{-4}4^5}{120} \approx 0.16
	\]
	For part~\ref{ex:cars}, the damage to a car after a hailstorm is Poisson with a mean
	of five dents. Let $N$ be the
	number of dents in a car after a hailstorm.
	\[ N \sim \text{Poisson}(\lambda = 5) \] 
	
	The probability that there will be two or more dents in a car after a hailstorm is 
	\begin{align*}
...
% To replace problem 27 ch 10 IMS
\item \emph{Lawn Chair Order Quantity}
Summer is approaching and a store is determining how many folding lawn chairs they should purchase from a supplier. The inventory managers only want to stock lawn chairs in the summer because that is when there is a high demand for the product. Each chair costs \$15\ for the store to purchase and can be sold for \$27.50.\  Any chairs left at the end of the summer can be sold on clearance for \$10.50.\ Based on data from past years, the seasonal demand for lawn chairs follows a normal distribution with $\mu = 205$ and $\sigma = 25$.

\begin{enumerate}
\item What is your recommended order quantity for the store?
\item What is the probability that the store will sell all the folding lawn chairs it orders before summer is over?
\item How would the economic order quantity change if the clearance price for the chairs was only \$8?\ What happens to the store's order quantity as the clearance price is reduced? 
\end{enumerate}

\begin{solution}
\bs Let $D$ be the summer demand for folding lawn chairs. We know that
$D \sim \mathcal{N}\left(\mu=205,~\sigma=25\right)$. The penalty
for ordering too few items is $c_u= 12.50$ and the penalty for
ordering too many items is $c_o= 4.50$. We want to find the
...
Q^{\ast} &= 220.75
\end{align*}
The grocery store should order 221 (or 220) folding lawn chairs. 

For part b) the probability that all chairs will sell is
\begin{align*}
...
Regarding part c), if the clearance price decreases to \$8\, then the cost of being under--stocked will still be  $c_u=12.50$, but the cost of being over will increase to $c_o=7.00$. The order quantity $Q^{\ast}$ will change such that 
\begin{align*}
...
\end{align*}
The new order quantity will be 214 chairs with a clearance price of \$8\. The order quantity will continue to decrease as the clearance price decreases because the cost of being over will continue to increase. 
