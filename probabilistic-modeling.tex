...
% this problem needs to be re-written.
\item \emph{Theme park waiting lines.} A small theme park, Expedited Expeditions, is known for having rides with short waiting times. A new ride has opened, and customers arrive to the ride according to a Poisson process with a mean of 20 arrivals per hour. Only one customer is allowed on the ride at a time, and customers can stay on the ride as long they wish. The length of time that each customer spends on the ride is exponentially distributed with a mean of 2.5 minutes. 
\begin{enumerate}
	\item Determine the average waiting time in line for the ride. \label{item:14a}
	\item Determine the average number of customers in line for the ride. \label{item:14b}
	\item Determine the probability that a customer will spend 15 minutes or less waiting in line and enjoying the ride. \label{item:14c}
\end{enumerate}
\begin{solution}
	\bs
	...
	For part~\ref{item:14a}, we wish to find $W_q$. 
	
	\[ W_q = \frac{\lambda}{\mu(\mu-\lambda)} = \frac{20}{24 \times(24-20)} = 5/24~\text{hours} = 12.5~\text{minutes} \] 
	
	For part~\ref{item:14b}, we can use Little's law to find $L_q$. 
	\[ L_q = \lambda \times W_q  = 20 \times 5/24 \approx 4~\text{customers}\]
	
	For part~\ref{item:14c}, we wish to find  the probability that a customer will spend 15 minutes or less in the system. Because 15 minutes = 1/4 hours, we will find P(W \leq 1/4).  
