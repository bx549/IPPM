...
A large passenger aircraft has $n$ engines. The performance of each engine is independent with failure probability $p$.
\begin{enumerate}
	\item Suppose only one appropriately working engine is required for a safe landing. What is the probability that the aircraft will experience an unsafe landing due to engine failure?  \label{ex:p1}
	\item Suppose two appropriately working engines are required for a safe landing. What is the probability that the aircraft will experience an unsafe landing due to engine failure? 
	\label{ex:p2}
\end{enumerate}

\begin{solution}
	\bs
	For part~\ref{ex:p1}, all $n$ engines must fail for the aircraft to experience an unsafe landing due to engine failure. Because failures are independent, the probability
	of an unsafe landing due to engine failure is $p^n$.
	
	For part~\ref{ex:p2}, let $X$ be the number of failed engines. $X$
	has a Binomial distribution with parameters $n$ and $p$. The probability
	of an unsafe landing due to engine failure is
	\begin{align*}
...
\item \emph{Memoryless property of the Exponential distribution.} A hair salon has two hairdressers who provide haircuts on a walk-in basis. Sam arrives to the salon and finds that both hairdressers are busy with customers. However, no one else is in line, so Sam will receive a haircut immediately after one of the hairdressers finishes the haircut they are working on. The service time distribution for both hairdressers is exponential with a mean of 15 minutes. What is the probability that both haircuts currently in progress will be completed before Sam’s haircut is completed?

\begin{solution}
	\bs
	By the memoryless property of the Exponential distribution, the
	remaining time for each customer currently receiving a haircut is identical.
	In particular, if we let $Y$ be the remaining service time of a customer, then
	the distribution of remaining time is
	\[ P(Y \leq y) = 1 - e^{-\lambda y} \] Since the two customers getting haircuts have
	the same distribution for remaining time, the probability that
	customer 1 finishes before customer 2 is 1/2 (likewise for
	customer 2 finishing before customer 1). When Sam enters service, the
	memoryless property still applies.  Regardless of how long the
	other customers have been getting their hair cut, Sam has the same distribution
	for remaining time. So, the probability that both haircuts in progress will be completed before Sam’s haircut is completed is 1/2.
\end{solution}
