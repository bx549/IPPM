...
% the customers?
\item \emph{Justifying a capital expense.} A small theme park, Expedited Expeditions, is known for having rides with short waiting times. A new ride has opened, and customers arrive to the ride according to a Poisson process with a mean of 20 arrivals per hour. Only one customer is allowed on the ride at a time, and customers can stay on the ride as long they wish. The length of time that each customer spends on the ride is exponentially distributed with a mean of 2.5 minutes. Management is willing to add an additional cart to the ride if both of the following conditions are satisfied:
1) the average number of customers in line is greater than three and 2) the probability that a customer spends 10 minutes or less at the ride is less than 0.65. Can an additional cart be justified?

\begin{solution}
	\bs
	The system in this problem is an M/M/1 queuing system. Customers arrive at the rate
	\[ \lambda = 20~\text{customers per hour} \]
	We know that the service rate is 
	\[ \mu = 24~\text{customers per hour} \]
	
	To check if the first condition is satisfied, we wish to find $L_q$. 
	
	\[ L_q = \frac{\lambda^2}{\mu(\mu-\lambda)} = \frac{20^2}{24(24-20)} = 25/6~\text{customers} \] 
	Thus, the first condition is satisfied.
	
	To check if the second condition is satisfied, we wish to find  the probability that a customer will spend 10 minutes or less in the system. Because 10 minutes = 1/6 hours, we will find $P(W \leq 1/6)$.  
	
	\[ P(W \leq t) = 1 - e^{-t(\mu-\lambda)}= 1 - e^{-1/6(24-20)} \approx 0.49\]
...
Patients arrive to an emergency room according to a Poisson process, with an arrival rate of 10 patients per hour. Before a patient can see a doctor, they must have their medical history reviewed and their vitals measured. Currently, the hospital has one nurse who performs both of these tasks for each patient. The time that each patient spends with the nurse is exponentially distributed with a mean of five minutes. An engineer at the hospital is considering two options for improving service.
\begin{compactenum}
	\item Hire a medical scribe to review each patient's medical history while the nurse takes vitals. The service rate would increase to 20 patients per hour with this improved single-server operation.
	\item Hire a second nurse who also reviews medical history and takes vitals for each patient. The two nurses would each have a service rate of 12 patients per hour with this two-server operation.
\end{compactenum}
...
\begin{solution}
	\bs We will look at the average number of patients in the queue $L_Q$ and the
	average time in queue $W_Q$ as metrics to judge improvement in
	system performance. Before any improvements are made,
	\[ L_q = \frac{\lambda^2}{\mu(\mu-\lambda)} = \frac{10^2}{12(12-10)} = 25/6~\text{patients} \]
	and
	...
	and if they add an additional nurse we
	need to use the formulas for a two-server operation. First,
	...
	and
	\[W_q = \frac{L_q}{\lambda}  \]
	Plugging values, I got $P_0=0.4118$, $L_q=0.1751$ patients, and $W_q=0.0175$ hours or $1.05$ minutes.
	Considering the relative improvement and the cost
	of adding a second nurse, I would recommend option a). That is, to hire a scribe to help the nurse. The average number in
	queue and the average time in queue appear to be acceptable for an emergency room setting.
\end{solution}
...
\begin{solution}
\bs
$c_o = \$10$ for each unsold copy, and $c_u = \$15$ per copy
for being short of actual demand. Let $D$ be a random variable
that represents demand for the class notes. We know that
\[ D \sim \text{Uniform}(200,\,250) \]
Let $Q^{\ast}$ be the optimal number of copies to produce.
Using the critical ratio, we want
\[ P\left(D \leq Q^{\ast}\right) = \frac{c_u}{c_u+c_o} = 0.6 \]
Using the definition of the CDF for the Uniform distribution,
\begin{align*}
  \frac{Q^{\ast} - 200}{250 - 200} &= 0.6 \\
  Q^{\ast} &= 0.6\times 50 + 200\\
  Q^{\ast} &= 230
\end{align*}
The bookstore should produce 230 copies.
\end{solution}

% This problem needs to be re-written. Can we make it more real-like
% than just saying "an item". The idea is to have an initial inventory
% policy that results in an unacceptable probability of stock out
% (either too low or too hi, then determine an inventory policy for
% the probability of stock out = 0.05 (or something reasonable.)
\item \emph{The risk of being out-of-stock.}
The daily demand for an item in a store follows a
  Normal distribution with mean 100 and standard deviation 5. It costs
  \$.024 per day to hold one item in inventory. The fixed cost to make
  a replenishment order is \$30, and the lead time to receive an order
  is 2 days.

\begin{enumerate}
\item Currently, the inventory policy for this item is to
order 300 units whenever the inventory drops to 220 units.
Determine the probability of a stock-out under this policy.

\item Recommend a better inventory policy for the store given
that the probability of a stock-out cannot exceed .05. Your policy
should include an economic order quantity and a re-order point.
\label{bb}
\end{enumerate}

\begin{solution}
\bs
The distribution of demand during lead time $X_m$ is
\[ X_m \sim \text{Normal}(\mu_m=200,\sigma_m=\sqrt{50}) \]
The probability of a stock-out is
\begin{align*}
  P(X_m \geq 220) &= P\left(z \geq \frac{220-\mu_m}{\sigma_m}\right) \\
                  &= 1 - P\left(z < \frac{220-200}{\sqrt{50}}\right) \\
                  &= 1 - P\left(z < 2.828\right) \\
                  &\approx .002
\end{align*}

For part~\ref{bb}. We can lower the total cost and keep the probability
of a stock-out to be less than .05. The optimal order quantity is
\[
  Q^{\ast} = \sqrt{\frac{2C_oD}{C_h}} = \sqrt{\frac{2\times 30\times 100}{.024}} = 500
\]
Without a buffer stock the optimal policy is to order 500 units
every $T_0=Q^{\ast}/D=5$ days. To compute the buffer stock, we want
\[
  P(X_m \geq B + 200) \leq 0.05
\]
or equivalently, we want $B \geq \sigma_m \times 1.645 \approx 12$.
The best policy is to order 500 units whenever the inventory
drops to $200+12=212$ units.
\end{solution}
