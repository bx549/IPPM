...
deviation 20. It costs \$.01042 per day to hold one item in
...
stock-out under this policy.

\item Recommend a better inventory policy for the school
given that the probability of a stock-out cannot exceed .01.
...
For part b, we can lower the total cost and keep 
the probability of a stock-out to be less than 
.02. The optimal order quantity is
\[
  Q^{\ast} = \sqrt{\frac{2C_oD}{C_h}} = 
  \sqrt{\frac{2\times 50\times 600}{.01042}} = 2400
\]
Without a buffer stock the optimal policy is to order 
\num{2400} milk cartons every $T_0=Q^{\ast}/D=4$ days. 
...
whenever the inventory drops to $1200+66=1266$ units.
\end{solution}
