...
% re-written by hannah
% Poisson distribution
\item \emph{Car dealership.} A car dealership keeps 200 used cars that
  are for sale in an uncovered parking lot. In the city where the
  dealership is located, hailstorms occur randomly throughout the
  month of June according to a Poisson distribution with a mean of 4
  hailstorms. During a hailstorm, the number of cars that are damaged
  follows a Poisson distribution with a mean of eight damaged cars per
  10 cars in the parking lot.
\begin{enumerate}
\item Determine the probability that there will be exactly 5
  hailstorms at the dealership in June. \label{ex:storms}
\item During a hailstorm, determine the probability that 170 or more
  cars will be damaged. \label{ex:cars}
\end{enumerate}

\begin{solution}
  \bs For part~\ref{ex:storms}, let $X$ be a random variable that represents
  the number of hailstorms in June. The number of hailstorms in June
  follows a Poisson distribution with a mean of 4
  hailstorms. Therefore, $X \sim \text{Poisson}(\lambda = 4)$.  The
  probability that there are exactly 5 hailstorms is
\[
P(X=5) = \frac{e^{-\lambda}\lambda^5}{5!} = \frac{e^{-4}4^5}{120} \approx 0.16
\]
For part~\ref{ex:cars}, the rate that cars are damaged is Poisson with a mean
of 8 damaged cars per 10 cars in the parking lot. Let $N$ be the
number of damaged cars in the parking lot when 200 cars are present.
...
\item \emph{Blood pressure screening.} High blood pressure is an
  ...
  with mean $\mu=135$ with standard deviation $\sigma=20$.  The
...
sysolic blood pressure, and recall that $\Phi(z)$ indicates the CDF of the
standard Normal distribution.
\begin{align*}
\text{very high}&
\quad 220\times(1-P(X<\mu+1.5\sigma))=220\times(1-\Phi(1.5)) \approx 15 \\
\text{high}&
\quad 220\times (P(X<\mu+1.5\sigma)-P(X<\mu))=220\times(\Phi(1.5)-\Phi(0)) \approx 95 \\
\text{average}&
\quad 220\times(P(X<\mu)-P(X<\mu-\sigma))=220\times(\Phi(0)-\Phi(-1))\approx 75 \\
\text{low}&
\quad 220\times(P(X<\mu-\sigma))=220\times\Phi(-1) \approx 35
\end{align*}
\end{solution}

% this problem needs to be re-written
% Lognormal distribution
\item \emph{Log-normal distribution.} A sporting goods company is designing a highly durable tennis racquet for competitive players. A manufacturing engineer at the company is tasked with determining which machine should be used to produce the racquets. The engineer has identified two machines that are suitable. Machine A is capable of manufacturing tennis racquets that have an expected life (in years) modeled by the log-normal distribution with parameters $\mu= 0.8$ and $\sigma= 0.25$. Machine B is capable of manufacturing tennis racquets that have an expected life (in years) modeled by the log-normal distribution with parameters $\mu= 0.9$ and $\sigma= 0.05$.
\begin{enumerate}
	\item Determine the mean life of a tennis racquet produced by machine A. \label{a1}
	\item Determine the mean life of a tennis racquet produced by machine B. \label{b1}
	\item Determine the probability that a tennis racquet produced by machine A will last more than 2.6 years. \label{a2}
	\item Determine the probability that a tennis racquet produced by machine B will last more than 2.6 years. \label{b2}
\end{enumerate}

\begin{solution}
	\bs 
	Let $X_A$ and $X_B$ represent the expected life of the racquets produced by machine A and machine B, respectively. It follows that  $X_A \sim \text{LogN}(\mu=0.8,\sigma=0.25)$ and $X_B
	\sim \text{LogN}(\mu=0.9,\sigma=0.05)$.
	
	For part~\ref{a1}, the mean expected life of the racquets produced by machine A is
	\[
	e^{\mu + \sigma^2/2} \approx 2.3~\text{years}
	\]
	For part~\ref{b1}, the mean expected life of the racquets produced by machine B is
	\[
	e^{\mu + \sigma^2/2} \approx 2.5~\text{years}
	\]
	
	For part~\ref{a2}, we wish to find $P(X_A > 2.6)$. It follows that
	\begin{align*}
		P(X_A > 2.6) &= P(\ln(X_A) > \ln(2.6)) \\
		&= P\left(Z > \frac{\ln(2.6)-0.8}{0.25}\right) \\
		&= P(Z > 0.622) \\
		&= 1-P(Z < 0.622) \\
		&= 0.267
	\end{align*}
	
	For part~\ref{b2}, we wish to find $P(X_B > 2.6)$. It follows that
	\begin{align*}
		P(X_B > 2.6) &= P(\ln(X_B) > \ln(2.6)) \\
		&= P\left(Z > \frac{\ln(2.6)-0.9}{0.05}\right) \\
		&= P(Z > 1.110) \\
		&= 1-P(Z < 1.110) \\
		&= 0.134
	\end{align*}
