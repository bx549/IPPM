...
$z_{ij}$) must be routed to node 5 (the discard node). To accomplish this, we can
add the following constraints to the existing model formulation.
...
Another idea is to split nodes 2 and 3 as depicted in the diagram
below. All flow into nodes 2a and 3a must be discarded, but is allowed
to help satisfy the demand requirements. 
...
\item \emph{Ordering lumber.}  Lakeland Builders always has a full schedule of
  building projects.  One of the most important resources required for
  their projects is lumber. To complete their projects, Lakeland
  has a constant demand for lumber of \num{5000} board feet
  per day.  There is a \$300\ delivery fee every time the company
  orders more lumber. It costs \$0.093\ per week for the company to
  store each unused board foot (7 days in one week). Every order has a
  lead time of 2 days to account for order processing and delivery
  time.

\begin{enumerate}
\item What is the optimal order quantity (in board feet) for Lakeland
  Builders?
\item How frequently should Lakeland order to replenish the
  lumber supply? (i.e. What is the cycle time?)
\item Lakeland currently has space to store \num{20000} board
...
\begin{solution}
  ...
  $C_h=\$.093$ per board foot per week, the lead time is $m=2$ days,
...
\[ Q^{\ast} = \sqrt{\frac{2DC_o}{C_h}} = \sqrt{\frac{2(\num{35000})(300)}{0.093}} \approx \num{15027}~\text{board feet.} \]

The cycle time associated with the economic order quantity is
\[ T^{\ast} = \frac{Q^{\ast}}{D} = \frac{\num{15027}}{\num{5000}} \approx 3 ~\text{days.} \]

No, Lakeland Builders should not invest in more space to store lumber
because the economic order quantity of \num{15027} board feet is less
