...
\item \emph{Maximum flow through a network.}  For the network shown
  below, formulate a linear programming problem to find the maximum
  flow from source node $s$ to sink node $t$.  The number on each edge
  indicates its capacity. The arrows indicate the direction of flow.
