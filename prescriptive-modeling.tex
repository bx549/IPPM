...
% This problem needs to be re-written, but we can simply change the
% numbers slightly
...
\begin{tikzpicture}
  \begin{axis}[ width=12cm, grid=both, axis x line=middle, axis y
    ...
    \addplot[domain=-1:4] {-3/2 + 1/2*x}; \addplot[domain=-1:4] {3
      - x}; \addplot[domain=-1:4,dashed] {3 - 1/2*x};
...
\begin{tikzpicture}
  \begin{axis}[ width=12cm, grid=both, axis x line=middle, axis y
    line=middle, title=, clip=false, ymin=-3, ymax=3, xmin=-3,
    xmax=3 ]

    \addplot[domain=-1:3] {-2+x}; \addplot[domain=-3:1] {2+x};
    \addplot[domain=-3:3,dashed] {-x}; \addplot[fill=gray, fill
    opacity=.25] coordinates { (0,0) (2,0) (3,1) (3,3) (1,3) (0,2)
    }\closedcycle;
