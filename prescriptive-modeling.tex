...
% this problem needs a new context. is there something in sports
% scheduling?  or in a mfg process a chemical solution (goo that is
% hard when cold flows when heated) holds parts to be machined in
% place. over a 4 week period the goo can be filtered or replaced
% (with new.) ???
...
% this problem needs a new context. but is a version of the well-known
% "caterer's" problem. what is a modern day version? a hotel that uses
% a service to launder sheets?
% i can create the LaTeX code for the network after we decide on the
% context.
\item \emph{Laundering coveralls.} A medical device company uses a cleanroom to assemble and inspect many of its products. Because the purpose of a cleanroom is to limit pollutants and contaminants, there are strict guidelines that must be followed. At this company, all employees and visitors who enter the cleanroom must wear a clean pair of coveralls. As a result, there must be enough clean coveralls at the beginning of each day for all employees or visitors who enter the cleanroom. The company outsources coverall cleaning to an outside laundry service. Normal laundry takes one full day at a cost of \$1.20 per pair of coveralls, and rapid laundry can be performed overnight at a cost of \$2.50 per pair of coveralls. Under normal circumstances, the current coverall supply of 300 coveralls is sufficient for complete dependence upon the normal laundry service. However, the normal laundry service will not be sufficient for the next three days because of a training program and audit that will take place. It is known that the coverall requirements for the next three days will be 265, 250, and 240, respectively. It is now midafternoon on the day before the training program and audit. 150 pairs of coveralls are clean, and 150 pairs of coveralls are ready for laundry. It is against standard operating procedure to have used coveralls linger overnight. The cleanroom will be closed on the day after the training program and audit, so all dirty coveralls on the third day can be sent to normal laundry. An industrial engineer wants to determine a plan for coverall laundering that will minimize total cost, while ensuring that all coverall demand is fulfilled, and all standard operating procedures are followed.
...
% this problem needs a new context. right now it could be an LP, but
% would like for it to be an IP, so the number of "units" to invest
% should probably be no more than about 100? could even be 50.
% we need a solution in GAMS. I have place my AMPL solution on github
% in the file "gambler.mod"
\item \emph{A maxi-min problem.} A college student has received a \$50 gift and is interested in investing in four different companies. One of three possible random events will occur in the next year that will impact the return on the investments. The return for each dollar invested in each of the four companies for each event is shown in the table below. For example, if the student invests \$1 in company 3 and event B occurs, the student will receive a return of \$7.  Investments must be made in increments of \$1. The student would like to maximize their minimum return when investing the \$50. Formulate an integer programming problem and obtain a solution.   \emph{Hint:} The
decision variables are the amounts to invest in each
company. Introduce another variable (say $w$) that represents the
...
\begin{solution}
	...
	in each company, where $i = 1,2,3,4$. Also, let $w$ represent
	the minimum return among the three events. Then the student wants to
	...
	\end{array}
	\]
	where $x_i \in \mathbb{Z}$-*, and $w$ is unrestricted.
\end{solution}
