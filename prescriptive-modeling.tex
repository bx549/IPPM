...
\item \emph{A maxi-min problem.} A college student has received a \$50
  gift and is interested in investing in four different companies. One
  ...
  dollar invested in each of the four companies for each event is
  shown in the table below. For example, if the student invests \$1 in
  company 3 and event B occurs, the student will receive a return of
  \$7.  Investments must be made in increments of \$1. The student
  would like to maximize their minimum return when investing the
  \$50. Formulate an integer programming problem and obtain a
  solution.  \emph{Hint:} The decision variables are the amounts to
  invest in each company. Introduce another variable (say $w$) that
...
\begin{solution}
	...
	\end{array}
	\]
	where $x_i$ is integer-valued and non-negative, and $w$ is unrestricted.
\end{solution}
