...
\begin{tikzpicture}
  \tikzstyle{level 1}=[sibling distance=43mm]
  ...
  child{node {B} 
    ...
    edge from parent node[below=10pt] {0} }
  child {node {B}
    child {node {\num{23000}}
    edge from parent node {0}}
    child {node {\num{25000}}
    ...
    child {node {\num{22750}}
    edge from parent node {3}}
    edge from parent node[above=1pt] {1} }
  child {node {B}
    ...
    = \num{26000}}
    edge from parent node {1}}
    edge from parent node[above=10pt] {2} }
  ;
\end{tikzpicture}
...
% this is the chess match example from Bertsekas.
...
\begin{tikzpicture}
\tikzstyle{level 1}=[sibling distance=65mm, level distance=20mm]
\tikzstyle{level 2}=[sibling distance=30mm, level distance=20mm]
\tikzstyle{level 3}=[sibling distance=15mm, level distance=25mm]
\tikzstyle{level 4}=[sibling distance=7mm,  level distance=30mm]

\node[rectangle,draw] {} [grow=right]
child{ node[circle,draw,label=above:.5366] {} 
  child{node[rectangle,draw] {}
    child{ node[circle,draw,label=above:.6975] {} 
      child{node {2--0} edge from parent node {win}}
      child{node {1--1} edge from parent node {lose} } edge from parent node {bold} }
    child{node[circle,draw,label=above:.945] {} 
      child{node {1.5--.5} edge from parent node {draw}}
      child{node {1--1} edge from parent node {lose}} edge from parent node {timid} } edge from parent node {win} }
  child{node[rectangle,draw] {}
    child{node[circle,draw,label=above:.2025] {} 
      child{node {1--1} edge from parent node {win} }
      child{node {0--2} edge from parent node {lose} } edge from parent node {bold} }
    child{node[circle,draw,label=above:0] {} 
      child{node {.5--1.5} edge from parent node {draw} }
      child{node {0--2} edge from parent node {lose} } edge from parent node {timid} } edge from parent node {lose} } edge from parent node {bold}  }
child{ node[circle,draw,label=above:.4253] {} 
  child{node[rectangle,draw] {}
    child{node[circle,draw,label=above:.45] {}
      child{node {1.5--.5} edge from parent node {win}}
      child{node {.5--1.5} edge from parent node {lose} } edge from parent node {bold} }
    child{node[circle,draw,label=above:.405] {}
      child{node {1--1} edge from parent node {draw}}
      child{node {.5--1.5} edge from parent node {lose}} edge from parent node {timid} } edge from parent node {draw} }
  child{node[rectangle,draw] {}
    child{node[circle,draw,label=above:.2025] {}
      child{node {1--1} edge from parent node {win} }
      child{node {0--2} edge from parent node {lose} } edge from parent node {bold} }
    child{node[circle,draw,label=above:0] {}
      child{node {.5--1.5} edge from parent node {draw} }
      child{node {0--2} edge from parent node {lose} } edge from parent node {timid} } edge from parent node {lose} } edge from parent node {timid} }
;
\end{tikzpicture}
...
\item \emph{A two-person zero-sum game.}  A professional football
  ...
  deal with costs related to poor publicity.  If the team decides to
...
\item \emph{Marketing strategies.} Two peanut butter companies,
  ...
  competitor and that the demand for peanut butter is constant, so a
  gain in sales for Doodle's is a loss of sales for Lola's. Each
  company has two possible options for packaging their product:
  standard packaging and sponsored packaging (e.g. put an action hero
  on the package).  If both companies choose to market only their
  sponsored packaging, Doodle's will gain an extra 2\%\ of the
  market's sales.  If both companies choose to market their standard
  packaging, Doodle's will loose 2\%\ of the market.  If Doodle's
  markets their sponsored product and Lola's markets their standard
  product, Doodle's will gain 10\%\ of the market.  If Lola's markets
  their sponsored product and Doodle's markets their standard product,
  Doodle's will gain 8\%\ of the market.
...
\item Formulate this decision problem as a two-person zero-sum game
  ...
  that they are able to change their packaging throughout the year,
  so a mixed strategy is possible.
\item Compute the value of the game. \label{val}
\item Suppose that a member of Doodle's marketing team quits her job
  and goes to work for Lola's. She tells her new co-workers about the
  strategy that Doodle's is planning to use. She is even able to tell
  them the probabiliites with which Doodle's will market their
  standard product and their sponsored product. Lola's marketing team
  now knows that Doodles is more likely to market the sponsored
  ...
  will improve their payoff. Is Lola's argument valid? In other words,
  does the value of the game change if Lola's knows Doodle's optimal
  strategy?
\label{knowledge}
\end{enumerate}
