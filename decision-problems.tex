...
% this problem needs a new context. we cannot use this example of a
% decision on where to go for the evening. the main idea is to
% reinforce the expected utility theorem ... that the utility of a
% "lottery" is its expected utility.
\item \emph{Friends and enemies.}  Utilities are the basis for making
  rational decisions.  Alice is making a decision on what to do for
  the evening. The three options and their inherent utilities are:
...
\begin{tabular}{lr}
  ...
  go to the club & 100\\
  go to the movie & 50\\
  watch a video at home & 50
\end{tabular}

With no other information, Alice would choose to go to the
club. However, there is some uncertainty (or risk) in Alice's
decision. The presence of Bob and/or Carol modify Alice's utilities.
Alice thinks Bob is a nuisance.
If she runs into Bob at the movie, her utility is decreased by 40.
If she runs into Bob at the club, her utility is decreased by 90.
On the other hand, Alice likes Carol. The presence of Carol at
either the movie or the club will increase Alice's utility by
a factor of 1.5 (after any disutility has been appplied).

A few more pieces of information are that Bob spends 40\% of
his time at the movie and 60\% of his time at the club. Carol
spends 75\% of her time at the movie and 25\% of her time
at the club.

Compute Alice's expected utilities for each option. What is the
``rational'' decision for Alice?

With this exercise, there are three points to be made:
\begin{enumerate}
\item Rational choice means that agents (like Alice) make decisions that maximize
  their utility (or payoff, or happiness). It doesn't mean that agents only
  care about themselves. Agents have preferences over states of the
  world. An agent might have a preference for making another agent
  happy, and that would be reflected in her utility for that state
  of the world.
