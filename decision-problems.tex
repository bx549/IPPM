...
% latex will treat minipage as a paragraph and indent the whole thing.
% so, here is my work-around: first use \noindent, then inside minipage,
% set the indentation for paragraphs.
\noindent
\begin{minipage}{.6\textwidth}
  \setlength{\parindent}{1pc} Sometimes taking a simple expected value
  makes sense.  Consider a gamble in which one of three outcomes will
  occur. The outcomes are worth \$1, \$5, and \$20 and the
  probabilities of the outcomes are .2, .5, and .3, respectively. For
  example, a wheel is spun and the probability of winning an amount
  corresponds to the area on the wheel.  The expected monetary value
...
important as the ``intrinsic worth'' of the money. For many (most)
people, an increase in the amount of money has increasing worth, but
at a decreasing rate. For example, given an amount of money $m$, the
logarithm captures the basic relationship between $m$ and its
``intrinsic worth''.
\begin{center}
...
\begin{axis}[
    ...
    title={The diminishing rate of the ``value'' of money}
  ]
...
probabililty $p$ or you receive outcome $y$ with probability
...
retirement, but Alice is not super-greedy; she wants to have
enough money to pay bills and take a few backpacking trips each year.
Her utility for an amount of money $m$ in the range \$0 to \$1 million
...
\begin{axis}[
    ...
    ylabel={$u(m)$},
    xticklabel={\$$\pgfmathprintnumber{\tick}$},
    title={Geometric interpretation of the gamble}
  ]
  \addplot[color=black,mark=none]
  coordinates {
(0,0.0669)(1000,0.0676)(2000,0.0683)(3000,0.069)(4000,0.0696)
...
We can represent the situation geometrically by drawing a line segment
...
from $u(\$\num{500000})$ to
$u(\$\num{800000})$.  The corresponding monetary value is the expected
value of the gamble, i.e. \$\num{600000}, and the corresponding
utility is the expected utility of the gamble, 6.51. With
\$\num{600000} in retirement funds, Alice's behavior changes.
We say that her utility function is risk-averse (or concave) in
the range from \$\num{500000} to \$\num{1000000} and that it is risk-seeking
(or convex) in the range from \$0 to \$\num{500000}. 
...
% written by Emily
\item \emph{Preference orderings and utility functions.}
    Suppose you have a goal to be a great long-distance runner.
    There are two choices involved with this goal in the form
    of $(x;y)$ where $x$ represents the number of miles you 
    are able to run in a day after you have trained for a month, 
    anywhere from zero to 30 miles, and $y$ represents the total 
    number of miles ran to train over the course of the month, 
    anywhere from zero to 500. Both $x$ and $y$ are continuous 
    distances, and $x \ge y$. In general you prefer to be able 
    to run farther with less training, but you always prefer to be able to
    run father no matter how much you have to train. For example,
  \[
