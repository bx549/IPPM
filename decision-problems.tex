...
% this problem needs a new context. can we find an industrial setting
% for this problem? 	
...
\begin{tikzpicture}
  ...
  child {node {c}
    child[sibling distance=10mm] {node[anchor=west] {1.25 - 1.00 + 1.20 + .60 = 2.05}
    edge from parent node {0}}
    child[sibling distance=10mm] {node[anchor=west] {1.25 - 1.00 - .25 + 1.20 + .60 + .40 = 2.20}
    edge from parent node {1}}
...
% this problem needs a new context. we cannot use the example of a lost dog.
% written by Emily
\item \emph{Contractors and the value of perfect information.} 
  ...
  bolt was in place. The workers do not want to throw away all
  bags of chips that the bolt could possibly be in, so they decide
  to search for the bolt. They have to decide if they should start 
  searching bags on pallet one or pallet two. If the bolt fell into
  a bag on pallet one and they search pallet one, there is a 0.5 
  chance of finding it by the end of the day. If the bolt fell into 
  a bag on pallet two and they search pallet two, there is a 0.8 chance
  that they find the bolt. The worker's prior probability that the bolt
  is in a bag on pallet two is 0.7 (usually they would have noticed the 
  missing bolt right away).

\begin{enumerate}
\item The workers only have time to search one pallet per day. 
  In which pallet, one or two, should they look for the bolt?  
  The workers want to maximize their
  probability of finding the bolt. The less searching they have to do,
  the more bags of chips are still able to be sold.
\item Suppose that at the end of the day when the workers go home,
  they have not found the bolt yet. Which pallet should the workers 
  search on day 2?
  \emph{Hint}: First update the prior probabilities that the bolt is in
  pallet one/two.
\item Suppose that on the second day, just before the workers begin to
  search for the bolt, a security guard approaches and tells the workers
  ...
  How much should the workers be willing to pay for this perfect 
...
that the bolt is in pallet two. Also, let $F_A$ represent the event that Oscar
...
& & $0.7$ & $0.3$ \\
& & $A$ & $B$  \\ \cline{3-4}
\multirow{2}{.9in}{Oscar} & \gtcol{Look in A} & \gtcol{.5} & \gtcol{0} \\ \cline{3-4}
& \gtcol{Look in B} & \gtcol{0} & \gtcol{.8} \\ \cline{3-4}
\end{tabularx}
\endgroup
\vspace{.2in}

Oscar's best option is to look in forest A. He will find the dog on the first
day with probability $0.7 \times 0.5 = 0.35$.

On the second day, Oscar's belief that Pablo is in forest A is updated to
\begin{align*}
...
and his best option is to look in forest B. Oscar will find Pablo with probability
\[ 0.462 \times 0.8 = 0.37\]

With perfect information, but \emph{before} receiving the information, Oscar's
expected value for the probability of finding Pablo is
\[ 0.5 \times 0.538 + 0.8 \times 0.462 = 0.639 \]
and in terms of probability ``units'', Oscar should be willing to pay
\[ 0.639 - 0.37 = 0.269 \]
for the information.
